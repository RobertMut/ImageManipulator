...
[TestFixture]
public class GetImageAfterThresholdQueryHandlerTests
{
    ...
    private GetImageAfterThresholdQueryHandler _handler;
    ...
    public async Task SetUp()
    {
        ...
        _imagePointOperationsServiceMock.Setup(x =>
            x.Thresholding(It.IsAny<Bitmap>(), It.IsAny<int>(), It.IsAny<bool>()))
            .Returns(_testImage);
        _handler = new GetImageAfterThresholdQueryHandler(_tabServiceMock.Object, _imagePointOperationsServiceMock.Object);
    }

    [AvaloniaTest]
    public async Task GetImageAfterThresholdQueryHandlerInvokesMethods()
    {
        Bitmap response = await _handler.Handle(new GetImageAfterThresholdQuery
        {
            Threshold = 30,
            ReplaceColour = true
        ...
        _imagePointOperationsServiceMock.Verify(x =>
                x.Thresholding(It.IsAny<Bitmap>(), It.Is<int>(x => x == 30), It.Is<bool>(x => x)),
            Times.Once);
    }
}